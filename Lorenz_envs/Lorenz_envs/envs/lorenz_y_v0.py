...
class Lorenzy(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None, g=10.0):
        ...
        self.clock = None
        self.isopen = True
        action_range_high = np.array([1.0, 1.0, 1.0], dtype=np.float32)
        action_range_low = np.array([-1.0, -1.0, -1.0], dtype=np.float32)
    ...
    def render(self, mode="human"):
        ...
        print ("with action:", x)
        print ('action:', u)
        for i, m, k in [(u, 'P', 'yellow'),(x, 'o', 'green'), (x_noaction_local, '^', 'blue')]:
        ...
        plt.title('Russell attractor')
        plt.draw()
        #plt.show(block=False)
        #self.collected_states = list()
        plt.savefig('Russell_ppo.png')
